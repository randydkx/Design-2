...
// 定义文法规则的文件名称
const string grammer_filename = "rule.txt";
// 词例表
...
production getProduction(string source){
    ...
    int pos_left_bracket = source.find('{');
    int pos_right_bracket = source.find('}');
    string semantic_string = source.substr(pos_left_bracket+1,pos_right_bracket - pos_left_bracket - 1);

    string right_side = source.substr(source.find(">")+1,pos_left_bracket-source.find(">"));
    ...
    // 处理语义动作
    // cout<<semantic_string<<endl;
    pro.semanticAction = semantic_string;
    if(semantic_string.find("print")!=string::npos){//规约是打印语义值
        pro.actionType = SEMANTIC_ACTION_TYPE::print;
    }else if(semantic_string.find("\'")!=string::npos && semantic_string.find(".lexval")!=string::npos){
        // 找到的是终结符值得赋值
        pro.actionType = SEMANTIC_ACTION_TYPE::constant_assign;
    }else if(semantic_string.find("\"")!=string::npos){//只含有非终结符，确定是变量赋值还是左右式赋值
        int count = 0;
        int pos = semantic_string.find("\"");
        while(pos!=string::npos)pos = semantic_string.find("\"",pos+1),count++;
        if(count==4){//还包括识别到npos的一次，只有一个非终结符号
            pro.actionType = SEMANTIC_ACTION_TYPE::parameter_assign;
        }else{
            assert(count==6);//只能处理二元操作符连接的文法
            symbol sym_l = pro.right[0];//默认二元操作符在中间
            symbol sym_op = pro.right[1];
            symbol sym_r = pro.right[2];
            int position[6];
            position[0] = semantic_string.find("\"");
            for(int i=1;i<6;i++)position[i]=semantic_string.find("\"",position[i-1]+1);
            string left_oprand = semantic_string.substr(position[0]+1,position[1]-position[0]-1);
            string right_oprand = semantic_string.substr(position[4]+1,position[5]-position[4]-1);
            if(left_oprand == sym_l.str && right_oprand == sym_r.str){
                pro.actionType = SEMANTIC_ACTION_TYPE::computing_assign_l_r;//二元操作符号的赋值类型：左 op 右->pro.left
            }else pro.actionType = SEMANTIC_ACTION_TYPE::computing_assign_r_l;//右 op 左->pro.left
        }
    }
    return pro;
}
...
// list:四元式序列
bool Processor::isIllegalList(vector<symbol> symList,vector<Token> tokenList,vector<string> &list){
    if(symList.size()==0)return true;
    for(int i=0;i<tokenList.size();i++)cout<<tokenList[i].value<<" ";
    cout<<"的LR(1)识别过程"<<endl;
    cout<<"步骤   状态栈   {符号栈 语义栈 额外栈}   输入字符串   ACTION  GOTO   语义动作   四元式动作表述"<<endl;
    int step = 0;//步骤
    int current_index = 0;//当前用于四元式变量索引的取值
    vector<int> stateStack;//状态栈
    vector<symbol> symbolStack;//符号栈ı
    vector<int> semanticStack;//语义栈
    vector<string> extraStack;//额外栈，主要记录当前符号栈中所用符号的变量名称

    list.clear();
    
    stateStack.push_back(0);//初始状态0
    symbolStack.push_back(symbol(FRONT_SERARCH,TYPE::front_search));//初始符号栈中有一个#
    semanticStack.push_back(-1);//不存在语义信息，将栈中的语义内容设置为int最大值，用来表示空值
    extraStack.push_back("_");//不存在附加变量符号
    ...
    for(int i=0;i<symbolStack.size();i++){
        cout<<"{"<<symbolStack[i].str;
        if(semanticStack[i]==-1)cout<<" _";
        else{
            cout<<" "<<semanticStack[i];
        }
        cout<<" "<<extraStack[i]<<"}";
    }
    ...
    while(true){
        ...
        if(action.type==TYPE_OF_ACTION_GOTO::PRODUCTION){
            ...
            if(new_action.type == TYPE_OF_ACTION_GOTO::PRODUCTION){
                ...
                cout<<pro.semanticAction<<"(语义动作)      ";
                if(pro.actionType==SEMANTIC_ACTION_TYPE::print){//在下一轮迭代之前更新好栈中的内容
                    cout<<semanticStack.back()<<"      ";//打印语义栈中的内容
                }else if(pro.actionType==SEMANTIC_ACTION_TYPE::computing_assign_l_r || 
                            pro.actionType==SEMANTIC_ACTION_TYPE::computing_assign_r_l){
                    int stackTop = semanticStack.size() - 1;
                    int value_l = semanticStack[stackTop - 2];
                    string operand = symbolStack[stackTop - 1].str;
                    int value_r = semanticStack[stackTop];
                    int operation_result;
                    if(operand == "*")operation_result = value_l * value_r;
                    else if(operand == "+"){
                        if(pro.actionType == SEMANTIC_ACTION_TYPE::computing_assign_l_r)
                            operation_result = value_l + value_r;
                        else operation_result = value_r + value_l;
                    }
                    else if(operand == "-"){
                        if(pro.actionType == SEMANTIC_ACTION_TYPE::computing_assign_l_r)
                            operation_result = value_l - value_r;
                    ...
                    string new_index = "t" + to_string( ++ current_index);
                    if(pro.actionType ==SEMANTIC_ACTION_TYPE::computing_assign_l_r)
                        list.push_back(string(new_index + ":=" + extraStack[stackTop-2] + operand + extraStack[stackTop]));
                    else list.push_back(string(new_index + ":=" + extraStack[stackTop] + operand + extraStack[stackTop-2]));
                    int count=3;
                    while(count--)semanticStack.pop_back(),extraStack.pop_back();
                    semanticStack.push_back(operation_result);//下一轮的语义栈更新完毕 
                    extraStack.push_back(new_index);
                }else if(pro.actionType == SEMANTIC_ACTION_TYPE::parameter_assign){
                    string new_index = "t" + to_string( ++ current_index);
                    int stackTop = extraStack.size() - 1;
                    if(symbolStack[stackTop].str!=")"){
                        list.push_back(string(new_index+":="+extraStack[stackTop]));
                        semanticStack.pop_back(),extraStack.pop_back();
                        int value = semanticStack[stackTop];
                        semanticStack.push_back(value);//值保持不变，实际上可以不用赋值
                        extraStack.push_back(new_index);
                    }
                    else{
                        list.push_back(string(new_index+":="+extraStack[stackTop-1]));
                        int count=3;
                        while(count--)semanticStack.pop_back(),extraStack.pop_back();
                        int value = semanticStack[stackTop-1];
                        semanticStack.push_back(value);//值保持不变，实际上可以不用赋值
                        extraStack.push_back(new_index);
                    }
                }else if(pro.actionType == SEMANTIC_ACTION_TYPE::constant_assign){//常量赋值
                    string value = tokenList[scanner - 1].value;
                    string new_index = "t" + to_string( ++ current_index);
                    int stackTop = extraStack.size() - 1;
                    list.push_back(string(new_index + ":=" + value));
                    extraStack.pop_back();
                    extraStack.push_back(new_index);
                }
                cout<<list.back()<<"    ";
            ...
            symbolStack.push_back(symbol(tokenList[scanner].value,TYPE::terminal));
            if(symList[scanner].str == SCIENTIFIC_FORMAT)
                semanticStack.push_back(atoi(tokenList[scanner].value.c_str()));
            else semanticStack.push_back(-1);
            extraStack.push_back("_");
            scanner++;
            Element new_action = this->Action[stateStack.back()][symList[scanner].str];
            // 打印一行结果
            cout<<++step<<" : ";
...
// 将表达式形式的四元式转换成括号形式的四元式
vector<string> formatTranslation(vector<string> list){
    ...
    for(auto element: list){
        ...
        int pos_two = -1;
        string operand;
        if((pos = element.find("*")) != string::npos || (pos = element.find("-")) != string::npos
         || (pos = element.find("+")) != string::npos){
            int pos1=element.find("t");
            int pos2=element.find("t",pos1+1);
            int pos3=element.find("t",pos+1);
            int pos4=element.find(":=");
            string result = element.substr(pos1,pos4-pos1);
            string arg1 = element.substr(pos2,pos-pos2);
            string arg2 = element.substr(pos3);
            string temp;temp.push_back(element[pos]);
            cout<<result<<":="<<arg1<<temp<<arg2<<endl;
            retList.push_back(string("( " + temp + " , " + arg1 + " , " + arg2 + " , " + result + " )"));
        }else if((pos = element.find("t")) != string::npos &&  (pos_two = element.find("t",pos+1)) != string::npos){
            int pos2 = element.find(":=");
            string result = element.substr(pos,pos2-pos);
            string arg1 = element.substr(pos_two);
            cout<<result<<":="<<arg1<<endl;
            retList.push_back(string("( := ,"+arg1 + " , " + "_ , " + result + " )"));
        }else{
            int pos = element.find(":=");
            string result = element.substr(0,pos);
            string scientific_value = element.substr(pos+2);
            cout<<result<<":="<<scientific_value<<endl;
            retList.push_back(string("( := ," + scientific_value + " , " + "_ , " + result + " )"));
        }
    }
...
bool isIllegalTokenList(){
    ...
    vector<string> list;
    bool success = processor.isIllegalList(symList,tokenList,list);
    
    vector<string> trans = formatTranslation(list);
    cout<<"变换之后的四元式："<<endl;
    for(auto str: trans)cout<<str<<endl;
